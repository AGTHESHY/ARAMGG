...
function filterGamesByQueue(games, queueId) {
  if (!queueId || queueId <= 0) return games
  const queueTag = `q_${queueId}`
  return games.filter(game => {
    const gameQueueId = game?.json?.queueId ?? 0
    if (gameQueueId === queueId) return true
    const tags = game?.metadata?.tags || []
    return tags.includes(queueTag)
  })
}

function calculateStats(games, puuid, queueId) {
  const filtered = filterGamesByQueue(games, queueId)
...
function tryInjectLobbyStats() {
  ...
  if (injected > 0) log(`Lobby stats injected for ${injected} elements`)
  return true
}
...
async function refreshLobbyStats() {
  ...
  log(`Refreshing lobby stats for ${members.length} members, queueId=${lobbyQueueId}`)
  ...
  await Promise.all(members.map(async member => {
    try {
      ...
      else log(`No stats for ${member.name} (puuid=${member.puuid})`)
    } catch (e) {
  ...
  log(`Lobby stats computed: ${nextStats.size}/${members.length} members`)
  tryInjectLobbyStats()
}
...
async function refreshLobbyMap() {
  try {
    ...
    const sig = JSON.stringify({
      m: (lobby?.members || []).map(m => m.puuid).sort(),
      q: lobby?.gameConfig?.queueId,
    })
    indexLobby(lobby)
    tryInjectLobbyStats()
    if (sig !== lastLobbySignature) {
      lastLobbySignature = sig
      log('Lobby changed, refreshing stats')
...
function startLobbyPolling() {
  ...
  lobbyPollTimer = setInterval(() => {
    if (lobbyRegistered) void refreshLobbyMap()
  }, 2000)
}
  ...
  if (enabled && !lobbyRegistered) {
    ...
    if (context?.socket?.observe) {
      try {
        ...
        log('Lobby socket observer registered')
      } catch (e) {
        warn('Lobby socket observe failed, falling back to polling only:', e)
      }
    } else {
      log('No socket available, using polling only for lobby')
    }
    startLobbyPolling()
    ...
    if (lobbySocketUnsub) { try { lobbySocketUnsub() } catch {} lobbySocketUnsub = null }
...
async function pollGameflowPhase() {
  try {
    ...
    log(`Gameflow phase changed: ${lastGameflowPhase} -> ${phase}`)
    lastGameflowPhase = phase
    if (phase === 'ChampSelect') {
      unregisterTierInjection()
      void applyChampSelectStats()
    } else if (phase === 'Lobby' || phase === 'None' || phase === 'EndOfGame') {
      unregisterTierInjection()
    }
  } catch {
    // ignore
...
function startChampSelectPolling() {
  if (champSelectPollTimer) clearInterval(champSelectPollTimer)
  champSelectPollTimer = setInterval(() => pollGameflowPhase(), 2000)
}
    ...
    if (context?.socket?.observe) {
      try {
        champSelectPhaseUnsub = context.socket.observe('/lol-gameflow/v1/gameflow-phase', (event) => {
          ...
          log(`Socket gameflow phase: ${phase}`)
          lastGameflowPhase = phase
          if (phase === 'ChampSelect') {
            unregisterTierInjection()
            void applyChampSelectStats()
        ...
        log('Champ select socket observers registered')
      } catch (e) {
        warn('Champ select socket observe failed, using polling only:', e)
      }
    } else {
      log('No socket available, using polling only for champ select')
    }
    startChampSelectPolling()
    void pollGameflowPhase()
    log('Champ select stats enabled')
  } else if (!enabled && champSelectPhaseUnsub) {
    try { champSelectPhaseUnsub() } catch {}
    champSelectPhaseUnsub = null
    if (champSelectUpdateUnsub) { try { champSelectUpdateUnsub() } catch {} champSelectUpdateUnsub = null }
    if (champSelectPollTimer) { clearInterval(champSelectPollTimer); champSelectPollTimer = null }
    unregisterTierInjection()
...
export function init(context) {
  ...
  if (context?.socket) log('Socket available')
  else warn('Socket NOT available in context')
}
