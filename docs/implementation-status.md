...
首阶段完成时没有启用自动同步或请求上游付费数据。后续已按用户授权部署测试服务器，完成真实上游接口验证，并启用正式中央缓存的定时断点续传；当前部署与进度见 [服务器部署说明](server-deployment.md)。服务端共享 Key Pool 已实现账户鉴权、加密保存、默认关闭共享、每日限额和撤销；客户端上传入口待 HTTPS 上线后开放。
